...
type publisherTestActivator struct {
	active bool
	events *[]string
}

func (a *publisherTestActivator) Active(string) bool {
	return a.active
}

func (a *publisherTestActivator) Activate(context.Context, string, func(string)) (*Handle, error) {
	*a.events = append(*a.events, "channel")
	a.active = true
	return nil, nil
}
...
func TestPublisherReconcilerActivatesChannelBeforeProvider(t *testing.T) {
	...
	reconciler, err := NewPublisherReconciler(store, activator, registry)
	if err != nil {
		t.Fatal(err)
	}
	...
	if want := []string{"channel", "publisher"}; !reflect.DeepEqual(events, want) {
...
func TestPublisherReconcilerReusesActiveChannel(t *testing.T) {
	events := []string{}
	store := publisherTestStore{
		publisher: pubsubmodel.Publisher{ID: "news", Channel: "news", Type: "news"},
		channel:   pubsubmodel.Channel{Name: "news"},
	}
	activator := &publisherTestActivator{active: true, events: &events}
	registry := NewPublisherRegistry()
	if err := registry.Register("news", publisherTestProvider{events: &events}); err != nil {
		t.Fatal(err)
	}
	reconciler, err := NewPublisherReconciler(store, activator, registry)
	if err != nil {
	...
	if err := reconciler.Reconcile(context.Background(), "news"); err != nil {
		t.Fatal(err)
	}
