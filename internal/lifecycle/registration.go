...
func NewRegistration(req RegisterRequest, now time.Time) (Registration, error) {
	...
	if req.PolicyCode != "" {
		raw, parseErr := strconv.ParseUint(req.PolicyCode, 10, 64)
		if parseErr != nil {
			return Registration{}, fmt.Errorf("parse policy_code: %w", parseErr)
		}
		...
		policy, policyErr := ratelimiter.NamedLifecyclePolicy(req.PolicyName)
		if policyErr != nil {
			return Registration{}, policyErr
		}
