...
func RuntimeRecordKey(namespace, instanceID, requestID string) string { return fmt.Sprintf("runtime:%s:%s:%s", cleanPart(namespace), cleanPart(instanceID), cleanPart(requestID)) }
func RuntimeRecordKeyScoped(scope keyspace.Scope, namespace, instanceID, requestID string) string { return scope.Name("logma", "runtime", namespace, instanceID, requestID) }
func runtimeIndexKey(namespace string) string { return "runtime_index:" + cleanPart(namespace) }
func runtimeIndexKeyScoped(scope keyspace.Scope, namespace string) string { return scope.Name("logma", "runtime-index", namespace) }

func cleanPart(value string) string { value = strings.TrimSpace(value); if value == "" { return "unknown" }; return strings.ReplaceAll(value, " ", "_") }
...
func normalizeDescriptors(handlers ChannelHandlers, descriptors []SubscriptionDescriptor) []SubscriptionDescriptor {
	...
	for _, descriptor := range descriptors { if descriptor.Channel == "" { continue }; if descriptor.ID == "" { descriptor.ID = cleanPart(descriptor.Channel) }; if descriptor.Callback == "" { descriptor.Callback = "internal:handler" }; byChannel[descriptor.Channel] = descriptor }
	for channel := range handlers { if _, ok := byChannel[channel]; ok { continue }; byChannel[channel] = SubscriptionDescriptor{ID: cleanPart(channel), Channel: channel, Callback: "internal:handler"} }
	result := make([]SubscriptionDescriptor, 0, len(byChannel)); for _, descriptor := range byChannel { result = append(result, descriptor) }; return result
}

type RuntimeLease struct { client *redis.Client; record RuntimeRecord; indexKey string; stop chan struct{}; closeOnce sync.Once }
...
func (cp ControlPlane) RegisterRuntime(ctx context.Context, invocation InvocationInfo, subscriptions []SubscriptionDescriptor, policies ...Policy) (*RuntimeLease, error) {
	...
	lease := &RuntimeLease{client:cp.Client, record:record, indexKey:indexKey, stop:make(chan struct{})}; go lease.heartbeat(ctx); return lease, nil
}

func (lease *RuntimeLease) heartbeat(ctx context.Context) { ticker:=time.NewTicker(runtimeHeartbeatEvery); defer ticker.Stop(); for { select { case <-ctx.Done(): return; case <-lease.stop:return; case now:=<-ticker.C: lease.record.UpdatedAt=now.UTC(); pipe:=lease.client.TxPipeline(); pipe.HSet(ctx,lease.record.Key,"updated_at",lease.record.UpdatedAt.Format(time.RFC3339Nano)); pipe.Expire(ctx,lease.record.Key,runtimeLeaseTTL); _,_=pipe.Exec(ctx) } } }
func (lease *RuntimeLease) Close() error { var closeErr error; lease.closeOnce.Do(func(){ close(lease.stop); ctx,cancel:=context.WithTimeout(context.Background(),5*time.Second); defer cancel(); pipe:=lease.client.TxPipeline(); pipe.Del(ctx,lease.record.Key); pipe.SRem(ctx,lease.indexKey,lease.record.Key); _,closeErr=pipe.Exec(ctx) }); return closeErr }
...
func ListRuntimeRecords(ctx context.Context, client *redis.Client, namespace string) ([]RuntimeRecord,error) { return listRuntimeRecordsAt(ctx,client,runtimeIndexKey(namespace)) }
