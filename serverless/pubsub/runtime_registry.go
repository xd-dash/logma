...
func RuntimeRecordKeyScoped(scope keyspace.Scope, namespace, instanceID, requestID string) string {
	return scope.Name("logma", "runtime", namespace, instanceID, requestID)
}
func runtimeIndexKey(namespace string) string { return "runtime_index:" + cleanPart(namespace) }
func runtimeIndexKeyScoped(scope keyspace.Scope, namespace string) string {
	return scope.Name("logma", "runtime-index", namespace)
}
...
func normalizeDescriptors(handlers ChannelHandlers, descriptors []SubscriptionDescriptor) []SubscriptionDescriptor {
	...
	for channel := range handlers {
		...
		byChannel[channel] = SubscriptionDescriptor{ID: cleanPart(channel), Channel: channel, Callback: "internal:handler"}
	}
...
func ListRuntimeRecords(ctx context.Context, client *redis.Client, namespace string) ([]RuntimeRecord, error) {
	return listRuntimeRecordsAt(ctx, client, runtimeIndexKey(namespace))
}
func ListRuntimeRecordsScoped(ctx context.Context, client *redis.Client, scope keyspace.Scope, namespace string) ([]RuntimeRecord, error) {
	return listRuntimeRecordsAt(ctx, client, runtimeIndexKeyScoped(scope, namespace))
}
func listRuntimeRecordsAt(ctx context.Context, client *redis.Client, indexKey string) ([]RuntimeRecord, error) {
...
func SaveRuntimeSubscriptionGroupScoped(ctx context.Context, client *redis.Client, scope keyspace.Scope, runtimeKey, groupID string) error {
	if _, err := keyspace.ParseScope(string(scope)); err != nil {
		return err
	}
...
func saveRuntimeSubscriptionGroup(ctx context.Context, client *redis.Client, scope keyspace.Scope, runtimeKey, groupID string) error {
	...
	for i, subscription := range record.Subscriptions {
		...
		if scope == "" {
			...
			key = scope.Name("logma", "subscription-group", groupID, id, subscription.Channel)
		}
